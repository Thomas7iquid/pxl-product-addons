...
if (!class_exists('WC_Product_Addons_Helper')) {
	class WC_Product_Addons_Helper {
		...
		public static function get_product_addons( $post_id, $prefix = false, $inc_parent = true, $inc_global = true ) {
			...
			// Remove addons which are variable on certain attribute and the attribute is not present in the current product
			foreach ( $addons as $key => $addon ) {
				$product_attributes = $product->get_attributes();
				if ( isset( $addon['variations'] ) && $addon['variations'] && isset( $addon['attribute'] ) ) {
					$attribute_obj = wc_get_attribute( intval( $addon['attribute'] ) );
					if ( ! is_null( $attribute_obj ) && ! isset( $product_attributes[ $attribute_obj->slug ] ) ) {
						unset( $addons[ $key ] );
					}
				}
			}
